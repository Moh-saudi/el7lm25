...
export const adminTranslations = {
  footer: {
    logoAlt: 'شعار المنصة',
    companyName: 'منصة الحلم الرياضية',
  ...
  dashboard: {
    ...
    subtitle: 'نظرة عامة وإحصائيات النظام',
    egp: 'جنيه',
    lastUpdate: 'آخر تحديث',
    loading: 'جاري تحميل لوحة التحكم متعددة العملات...',
    refreshRates: 'تحديث أسعار الصرف',
    loadingCurrencyStats: 'جاري تحميل إحصائيات العملات...',
    ...
    // مفاتيح مطلوبة في الواجهة كأسماء مختصرة
    currencySystem: {
      title: 'نظام العملات المتعددة',
      baseCurrency: 'العملة الأساسية',
      supported: 'عملات مدعومة',
    ...
    // أسماء كما تُستدعى في الكود
    recentActivity: {
      title: 'النشاط الحديث',
      subtitle: 'آخر أحداث النظام'
    },
    activities: {
      bulkPaymentSAR: 'دفعة جماعية جديدة بالريال السعودي (SAR)',
      exchangeRatesUpdated: 'تم تحديث أسعار الصرف - أكثر من 60 عملة محدثة',
      newPlayers: 'لاعبين جدد انضموا من دول مختلفة',
      usdPayment: 'دفعة بالدولار الأمريكي (تم التحويل تلقائياً للجنيه المصري)'
    },
    ...
    systemStatus: {
      title: 'حالة النظام',
      subtitle: 'جميع الأنظمة تعمل بشكل طبيعي',
      currencyAPI: 'واجهة برمجة العملات',
      connected: 'متصل',
      paymentSystem: 'نظام المدفوعات',
      active: 'نشط',
    ...
    errors: {
      exchangeRates: 'خطأ في تحديث أسعار الصرف',
      users: 'خطأ في جلب المستخدمين',
      currencyStats: 'خطأ في جلب إحصائيات العملات',
      stats: 'خطأ في جلب الإحصائيات'
    }
  },
...
export function t(key: string, fallback?: string): string {
  ...
  return (current as string) || fallback || key;
}

// دالة للحصول على الترجمة مع إدراج متغيرات
export function tWithVars(key: string, vars: Record<string, unknown> = {}, fallback?: string): string {
