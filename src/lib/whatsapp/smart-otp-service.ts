...
class SmartOTPService {
  ...
  async sendOTP(config: OTPConfig): Promise<OTPResponse> {
    ...
    if (!this.validateConfig()) {
      return { 
        success: false, 
        error: 'BeOn token not configured' 
      };
    }

    const method = this.getSendingMethod(config.country);
    console.log('📱 Sending method determined:', method);

    try {
      switch (method) {
        case 'both':
          return await this.sendBothWhatsAppAndSMS(config);
        case 'whatsapp':
          return await this.sendWhatsAppOnly(config);
        case 'sms':
          return await this.sendSMSOnly(config);
        default:
          return await this.sendWhatsAppOnly(config);
      }
    } catch (error: any) {
      console.error('❌ Smart OTP error:', error);
      return { 
        success: false, 
        error: error.message 
      };
  ...
  private async sendBothWhatsAppAndSMS(config: OTPConfig): Promise<OTPResponse> {
    ...
    try {
      ...
      if (whatsappResult.success && smsResult.success) {
        console.log('✅ Both WhatsApp and SMS sent successfully');
        return {
          success: true,
          otp: whatsappResult.otp || smsResult.otp,
          message: 'تم إرسال رمز التحقق عبر WhatsApp و SMS',
          method: 'both'
        };
