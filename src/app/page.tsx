      ...
      <section id="jobs" className="py-20 bg-gradient-to-br from-white to-blue-50">
        <div className="container px-4 mx-auto">
          <div className="text-center mb-12 flex flex-col items-center">
            ...
            <p className="text-gray-600 mt-2">{t('home.sections.jobs.subtitle') || 'انضم إلى فريق el7lm وكن جزءاً من رحلتنا'}</p>
          </div>
          ...
          <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
            {[
              ...
              <div key={role.key} className="p-6 bg-white rounded-2xl shadow-lg hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1">
                <div className="flex items-center justify-between mb-4">
                  <h3 className="text-2xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-blue-600 to-teal-500 flex items-center gap-2">
                    <span>{role.icon}</span>
                    <span>{t(`home.sections.jobs.roles.${role.key}`)}</span>
                  </h3>
                </div>
                <button
                  onClick={() => router.push(`/careers/apply?role=${role.key}`)}
                  className="w-full py-3 text-white transition-all transform bg-gradient-to-r from-blue-600 to-teal-500 rounded-xl hover:shadow-lg hover:scale-[1.02]"
                >
      ...
      <footer className="bg-gradient-to-br from-[#2d1857] to-[#0a2342] text-white py-14 mt-20 relative">
        <div className="container px-4 mx-auto">
          ...
          <div className="pt-8 mt-8 text-center text-gray-200 border-t border-violet-900">
            <p>© {new Date().getFullYear()} el7lm. {t('home.sections.footer.copyright')}</p>
          </div>
        </div>
