// ============================================
// نظام عرض الأسعار الثابتة والدفع الإلكتروني
// ============================================
// 
// كيف يعمل النظام:
// 1. الأسعار ثابتة لكل الدول (70، 120، 180)
// 2. يعرض الأسعار مع رمز العملة المحلية فقط
// 3. عند الدفع بالبطاقة: يتم التحويل خفياً للجنيه المصري
// 4. المستخدم يرى فقط الأسعار الثابتة بعملته المحلية
// 5. نظام تحديث أسعار الصرف يستخدم فقط للتحويل عند الدفع
//
// مثال:
// - مستخدم سعودي: يرى السعر 70 ر.س، 120 ر.س، 180 ر.س
// - عند الدفع: يتم تحويل 70 ر.س إلى جنيه مصري (خفياً)
...
// قاموس المدن حسب الدولة لتصحيح البيانات الخاطئة
...
// دالة للحصول على الدولة الصحيحة من المدينة
...
export default function PaymentPage() {
        ...
        try {
          ...
          if (city) {
            ...
            if (countryFromCity) {
              ...
              console.log(`🔧 تصحيح تلقائي: المدينة "${city}" تنتمي للدولة ${countryFromCity}`);
            }
          }
          
          // جلب الدولة من Firestore أو من user أو قيمة افتراضية
          let normalizedCountry = normalizeCountry(correctedCountry)
    ...
    try {
      ...
      if (data.success) {
        ...
        if (data.isTestMode) {
          ...
          setSuccessMessage('تم الدفع بنجاح (وضع اختبار)! سيتم تفعيل اشتراكك قريباً.');
          setSuccess(true);
          setTimeout(() => {
            router.push('/dashboard/payment/success');
          }, 2000);
        } else {
          // بدء عملية الدفع باستخدام GeideaCheckout للوضع الحقيقي فقط
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      // حفظ بيانات الدفع - استخدم السعر الثابت
      const displayAmount = PACKAGES[selectedPackage]?.price || 0;
