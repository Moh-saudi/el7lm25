...
export default function AcademyPlayersPage() {
  ...
  const loadPlayers = async () => {
    try {
      ...
      // جرب الحقلين academy_id و academyId
      const q1 = query(collection(db, 'players'), where('academy_id', '==', user?.uid));
      const q2 = query(collection(db, 'players'), where('academyId', '==', user?.uid));
      ...
      console.log('📊 نتائج البحث - academy_id:', snapshot1.size, 'مستندات');
      console.log('📊 نتائج البحث - academyId:', snapshot2.size, 'مستندات');
