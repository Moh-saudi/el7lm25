...
export default function AcademyPlayersPage() {
  ...
  const loadPlayers = async () => {
    try {
      setLoading(true);
      console.log('🔍 محاولة جلب اللاعبين للأكاديمية:', user?.uid);
      console.log('🔍 إيميل الأكاديمية:', user?.email);
      ...
      const queries = [
        ...
        // البحث بـ academyId
        query(collection(db, 'players'), where('academyId', '==', user?.uid)),
        // البحث بـ created_by
        query(collection(db, 'players'), where('created_by', '==', user?.uid)),
        // البحث بـ created_by_type = 'academy'
        query(collection(db, 'players'), where('created_by_type', '==', 'academy')),
      ];
      ...
      console.log('  - academy_id:', snapshots[0].size, 'مستندات');
      console.log('  - academyId:', snapshots[1].size, 'مستندات');
      console.log('  - created_by:', snapshots[2].size, 'مستندات');
      console.log('  - created_by_type=academy:', snapshots[3].size, 'مستندات');
      if (snapshots[4]) {
      ...
      // فلترة اللاعبين المرتبطين بهذه الأكاديمية فقط
      console.log('🔍 بدء فلترة اللاعبين للأكاديمية. معرف الأكاديمية:', user?.uid);
      console.log('🔍 إيميل الأكاديمية:', user?.email);
      
      const filteredDocs = uniqueDocs.filter(doc => {
        const data = doc.data();
        const matches = {
          academy_id: data.academy_id === user?.uid,
          academyId: data.academyId === user?.uid,
          created_by: data.created_by === user?.uid,
          created_by_type_academy: data.created_by_type === 'academy' && data.created_by === user?.uid,
          email_match: data.official_contact?.email === user?.email
        };
        
        const isMatch = matches.academy_id || matches.academyId || matches.created_by || matches.created_by_type_academy || matches.email_match;
        
        console.log(`📄 فحص اللاعب: ${data.full_name || data.name}`, {
          player_academy_id: data.academy_id,
          player_academyId: data.academyId,
