...
export default function AgentPlayersPage() {
  ...
  const loadPlayers = async () => {
    try {
      setLoading(true);
      console.log('🔍 محاولة جلب اللاعبين للوكيل:', user?.uid);
      console.log('🔍 إيميل الوكيل:', user?.email);
      
      // جرب جميع الطرق الممكنة للبحث
      const queries = [
        // البحث بـ agent_id
        query(collection(db, 'players'), where('agent_id', '==', user?.uid)),
        // البحث بـ agentId
        query(collection(db, 'players'), where('agentId', '==', user?.uid)),
        // البحث بـ created_by
        query(collection(db, 'players'), where('created_by', '==', user?.uid)),
        // البحث بـ created_by_type = 'agent'
        query(collection(db, 'players'), where('created_by_type', '==', 'agent')),
      ];
      ...
      console.log('  - agent_id:', snapshots[0].size, 'مستندات');
      console.log('  - agentId:', snapshots[1].size, 'مستندات');
      console.log('  - created_by:', snapshots[2].size, 'مستندات');
      console.log('  - created_by_type=agent:', snapshots[3].size, 'مستندات');
      if (snapshots[4]) {
      ...
      // فلترة اللاعبين المرتبطين بهذا الوكيل فقط
      console.log('🔍 بدء فلترة اللاعبين للوكيل. معرف الوكيل:', user?.uid);
      console.log('🔍 إيميل الوكيل:', user?.email);
      
      const filteredDocs = uniqueDocs.filter(doc => {
        const data = doc.data();
        const matches = {
          agent_id: data.agent_id === user?.uid,
          agentId: data.agentId === user?.uid,
          created_by: data.created_by === user?.uid,
          created_by_type_agent: data.created_by_type === 'agent' && data.created_by === user?.uid,
          email_match: data.official_contact?.email === user?.email
        };
        
        const isMatch = matches.agent_id || matches.agentId || matches.created_by || matches.created_by_type_agent || matches.email_match;
        
        console.log(`📄 فحص اللاعب: ${data.full_name || data.name}`, {
          player_agent_id: data.agent_id,
          player_agentId: data.agentId,
