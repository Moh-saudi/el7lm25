...
export default function AgentPlayersPage() {
  ...
  const loadPlayers = async () => {
    try {
      ...
      // جرب الحقلين agent_id و agentId
      const q1 = query(collection(db, 'players'), where('agent_id', '==', user?.uid));
      const q2 = query(collection(db, 'players'), where('agentId', '==', user?.uid));
      ...
      console.log('📊 نتائج البحث - agent_id:', snapshot1.size, 'مستندات');
      console.log('📊 نتائج البحث - agentId:', snapshot2.size, 'مستندات');
