  ...
  const fetchTrainerData = useCallback(async () => {
    ...
    try {
      ...
      const mergedData = {
        ...initialTrainerData,
        ...data,
        full_name: data.full_name || data.name || userData?.name || 'مدرب رياضي',
        phone: data.phone || userData?.phone || '',
        email: data.email || userData?.email || '',
        profile_photo: getSupabaseImageUrl(data.profile_photo || initialTrainerData.profile_photo),
        coverImage: getSupabaseImageUrl(data.coverImage || initialTrainerData.coverImage),
        social_media: {
  ...
  const handleImageUpload = async (file: File, type: 'profile' | 'document' | 'cover') => {
    ...
    try {
      ...
      if (file.size > 5 * 1024 * 1024) {
        ...
        console.error(`❌ ملف كبير جداً: ${fileSizeMB}MB (الحد الأقصى: 5MB)`);
        toast.error(`حجم الملف كبير جداً! 
        📊 حجم الملف: ${fileSizeMB} ميجابايت
        ⚠️ الحد الأقصى المسموح: 5 ميجابايت
        
        💡 نصائح لتقليل حجم الصورة:
        • استخدم أدوات ضغط الصور online
        • قم بتغيير حجم الصورة إلى 800x800 بكسل
        • احفظ بصيغة JPG بدلاً من PNG`, {
          duration: 8000,
          style: {
            maxWidth: '400px',
            fontSize: '14px',
