  ...
  const loadPlayers = async () => {
    try {
      ...
      const queries = [
        ...
        // البحث بـ clubId
        query(collection(db, 'players'), where('clubId', '==', user?.uid)),
        // البحث بـ created_by
        query(collection(db, 'players'), where('created_by', '==', user?.uid)),
        // البحث بـ created_by_type = 'club'
        query(collection(db, 'players'), where('created_by_type', '==', 'club')),
      ];
      ...
      console.log('  - club_id:', snapshots[0].size, 'مستندات');
      console.log('  - clubId:', snapshots[1].size, 'مستندات');
      console.log('  - created_by:', snapshots[2].size, 'مستندات');
      console.log('  - created_by_type=club:', snapshots[3].size, 'مستندات');
      if (snapshots[4]) {
      ...
      // فلترة اللاعبين المرتبطين بهذا النادي فقط
      console.log('🔍 بدء فلترة اللاعبين للنادي. معرف النادي:', user?.uid);
      console.log('🔍 إيميل النادي:', user?.email);
      
      const filteredDocs = uniqueDocs.filter(doc => {
        const data = doc.data();
        const matches = {
          club_id: data.club_id === user?.uid,
          clubId: data.clubId === user?.uid,
          created_by: data.created_by === user?.uid,
          created_by_type_club: data.created_by_type === 'club' && data.created_by === user?.uid,
          email_match: data.official_contact?.email === user?.email
        };
        
        const isMatch = matches.club_id || matches.clubId || matches.created_by || matches.created_by_type_club || matches.email_match;
        
        console.log(`📄 فحص اللاعب: ${data.full_name || data.name}`, {
          player_club_id: data.club_id,
          player_clubId: data.clubId,
      ...
      // إذا لم نجد أي لاعبين، دعنا نبحث في جميع اللاعبين للتشخيص
      if (playersData.length === 0) {
        console.log('⚠️ لم يتم العثور على لاعبين، جاري البحث في جميع اللاعبين للتشخيص...');
        const allPlayersQuery = query(collection(db, 'players'));
        const allPlayersSnapshot = await getDocs(allPlayersQuery);
        console.log('📊 إجمالي اللاعبين في النظام:', allPlayersSnapshot.size);
        
        if (allPlayersSnapshot.size > 0) {
          console.log('🔍 عينة من اللاعبين في النظام:');
          allPlayersSnapshot.docs.slice(0, 10).forEach((doc, index) => {
            const data = doc.data();
            console.log(`  ${index + 1}. ${data.full_name || data.name || 'بلا اسم'}`, {
              id: doc.id,
              club_id: data.club_id,
              clubId: data.clubId,
              created_by: data.created_by,
              created_by_type: data.created_by_type,
              official_contact_email: data.official_contact?.email
            });
          });
        }
      }
