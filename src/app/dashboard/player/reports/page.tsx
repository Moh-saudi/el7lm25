...
function PlayerReportPage() {
  ...
  useEffect(() => {
    ...
    console.groupEnd();
  }, [searchParams, user, loading, authError]);
      ...
      for (const [key, orgType] of Object.entries(ORGANIZATION_TYPES)) {
        ...
        try {
          const userDoc = await retryOperation(async () => {
            return await getDoc(doc(db, orgType.collection, user.uid));
          }, 3, 1000);
          ...
          console.log(`⚠️ فشل في جلب من ${orgType.collection}:`, collectionError);
          // لا نوقف الحلقة، نتابع البحث في المجموعات الأخرى
          continue;
        }
      }
    } catch (error) {
    ...
    try {
      ...
      for (const [key, orgType] of Object.entries(ORGANIZATION_TYPES)) {
        ...
        if (orgId) {
          ...
          try {
            const orgDocSnap = await retryOperation(async () => {
  ...
  useEffect(() => {
    ...
    const fetchPlayerData = async () => {
      ...
      try {
        ...
        if (!playerId) {
          ...
          return;
              ...
              console.log('🔍 محاولة جلب بيانات اللاعب:', playerId);
      
      // استخدام retry logic لـ Firestore operations
      ...
      // أولاً: البحث في users collection للاعبين المحولين (أولوية عليا)
      let playerDoc;
      let dataSource = 'users';
      
      try {
        const result = await fetchPlayerDoc('users', playerId);
        playerDoc = result.docSnap;
        dataSource = result.collectionName;
              } catch (error) {
          console.log('⚠️ فشل في جلب من users collection:', error);
          // نتابع البحث في المجموعات الأخرى
        }
        
        // إذا لم يوجد في users، جرب players collection
        if (!playerDoc?.exists()) {
          try {
            console.log('🔍 محاولة جلب من players collection:', playerId);
            const result = await fetchPlayerDoc('players', playerId);
            playerDoc = result.docSnap;
            dataSource = result.collectionName;
          } catch (error) {
            console.log('⚠️ فشل في جلب من players collection:', error);
            // نتابع البحث في المجموعات الأخرى
          }
        }
        
        // إذا لم يوجد في players، جرب player collection
        if (!playerDoc?.exists()) {
          try {
            console.log('🔍 محاولة جلب من player collection:', playerId);
            const result = await fetchPlayerDoc('player', playerId);
            playerDoc = result.docSnap;
            dataSource = result.collectionName;
          } catch (error) {
            console.log('⚠️ فشل في جلب من player collection:', error);
            // نتابع البحث في المجموعات الأخرى
          }
        }

        if (!playerDoc.exists()) {
          console.warn('⚠️ لم يتم العثور على اللاعب في أي collection:', playerId);
        ...
        console.log(`✅ تم العثور على اللاعب في ${dataSource}:`, {
          playerName: data.full_name || data.name,
          accountType: data.accountType,
          hasClubId: !!data.club_id,
          convertedFromDependent: data.convertedFromDependent,
          source: dataSource
        });
