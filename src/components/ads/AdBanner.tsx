      ...
      {visibleAds.map((ad, index) => (
        ...
        >
          ...
          <Button
            variant="ghost"
            size="sm"
            className="absolute top-4 left-4 z-20 h-8 w-8 p-0 rounded-full bg-white/90 hover:bg-white shadow-lg opacity-0 group-hover:opacity-100 transition-all duration-300"
            onClick={(e) => {
              e.stopPropagation();
              dismissAd(ad.id);
            }}
