 ...
 */
export function sanitizePlayerImages(playerData: any) {
  ...
  if (sanitized.avatar_url) {
    sanitized.avatar_url = sanitizeImageUrl(sanitized.avatar_url);
  }
  
  if (sanitized.images && Array.isArray(sanitized.images)) {
    sanitized.images = sanitizeImageUrls(sanitized.images);
  }
